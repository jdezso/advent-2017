...
public class DayFiveB
{
    static private int step(final List<Integer> instructions, int idx)
    {
        ...
        if (instruction >= 3)
        {
            instructions.set(idx, instruction - 1); // Decrement high "used" instruction
        }
        else
        {
            instructions.set(idx, instruction + 1); // Increment low "used" instruction
        }
